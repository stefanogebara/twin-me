...
vi.mock('../../../../api/_app/services/money/store.js', async (orig) => ({ ...(await orig()), listTransactions: async () => [{ id: 't1', occurred_at: '2026-09-19T10:00:00Z', amount: -12.5, currency: 'EUR', merchant_key: 'cafe', merchant_raw: 'Cafe' }], months: async () => [], recurringSeries: async () => [], listReadings: async () => [], listFacts: async () => [], questionsFor: async () => ({ opening: [], fromLedger: [], answered: 0 }), listPlaces: async () => [], listBankAccounts: async () => [], userLanguage: async () => 'en', categorySpend: async () => ({ groups: [] }), saveChatTurn: async () => null, listChatTurns: async () => [] }));
vi.mock('../../../../api/_app/services/money/forecastService.js', () => ({ forecast: async () => null, months: async () => [] }));
import { answer } from '../../../../api/_app/services/money/chat.js';
