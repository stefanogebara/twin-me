...
export function silenceDeltas(profiles, { now, away = [] } = {}) {
  ...
  for (const p of profiles || []) {
    ...
    out.push({
      ...
      numbers: { merchant_key: p.merchant_key, name: p.name, first_seen: p.first_seen, days_since: days, away_days: awayDays, usual_gap_days: p.median_gap_days, times: p.times, typical_amount: p.typical_amount },
      receipts: [],
...
export function weekdayDelta(transactions, { now, isSpending } = {}) {
  ...
  return {
    ...
    numbers: { weekday: wd, current: here, usual, previous, count: todayRows.length },
...
export function paceDelta(transactions, { now, isSpending } = {}) {
  ...
  return {
    ...
    numbers: { current: here, usual, ratio: usual ? r2(here / usual) : null, weekly, count: nowRows.length },
    receipts: receipts(nowRows),
