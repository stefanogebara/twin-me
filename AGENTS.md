...
bank feed / phone / email inbox / statement
   ...
   -> routes/money.js (53 handlers) -> store.js (persistence + orchestration)
...
(atomic plan/commit), `store.js` (persistence; 47 exports, being split), `projection.js`,
...
`inbox.js` (receipts), `twinBridge.js` (what the twin is told). Routes: `routes/money.js`,
`routes/cron-money-pull.js` (hourly; bank read yields to the loop at 30 s).
...
RLS is on for all of them (a `DO ... EXECUTE` block, so static scans undercount);
`anon` is revoked on every money table.

### The twin — what still exists (secondary)

Measured 2026-09-19: 34 twin-chat memories by 2 users in 30 days, against 3,214
reflections and 160 proactive insights generated by crons in the same window. The
machinery runs for nobody; parking it is decision D1 in the tracker.

What exists, with its file: memory stream (`memoryStreamService.js`, `user_memories`,
`search_memory_stream` RPC; three-factor retrieval); expert reflections
(`reflectionEngine.js`, five personas, `MAX_REFLECTION_DEPTH` 1); observation ingestion
(`observationIngestion.js`, cron `*/30`); twin summary (`twinSummaryService.js`,
4-hour TTL); proactive insights (`proactiveInsights.js`, nudges with outcome
evaluation); goal tracking (`goalTrackingService.js`, `routes/goals.js`); personality
profile + prompt builder (`personalityProfileService.js`, `personalityPromptBuilder.js`;
OCEAN is never written and prints 0.5 defaults); memory links
(`memoryLinksService.js`); LLM wiki (`wikiCompilationService.js`, flag `llm_wiki`,
default off; `twinPromptAssembly.js` nulls it); twin chat (`routes/twin-chat.js`,
`twinContextBuilder.js`, `chatRouter.js`).

Gone (delete any reference you find): `neurotransmitterService.js`, `neuropilRouter.js`,
`saliencyReplayService.js`, `personalityReranker.js`, `personalityDriftService.js`,
`cron-memory-saliency-replay.js`.
