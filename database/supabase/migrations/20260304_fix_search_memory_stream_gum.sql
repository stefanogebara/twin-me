-- ============================================================================
-- Fix search_memory_stream: Return GUM columns + type-differentiated decay
-- Date: 2026-03-04
--
-- Changes from original (20260222):
-- 1. Returns `confidence`, `decay_rate`, `retrieval_count` columns
-- 2. Uses per-type decay_rate instead of flat p_decay_factor:
--    POWER(0.995, hours / COALESCE(decay_rate, type_default))
--    Type defaults: conversation=3, platform_data=7, fact=30, reflection=90
-- 3. p_decay_factor kept in signature for API compat but no longer used
-- ============================================================================

-- Must drop first because return type changed (added confidence, decay_rate, retrieval_count)
...
BEGIN
  ...
  WITH candidates AS (
    SELECT
      ...
      -- Type-differentiated recency decay using per-memory decay_rate
      -- decay_rate is stability in days; we convert hours to "decay units"
      -- Higher decay_rate = slower decay (reflections last longer than conversations)
      POWER(
        0.995,
