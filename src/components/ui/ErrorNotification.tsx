...
const ErrorNotification: React.FC = () => {
  ...
  const getStyles = (type: ErrorInfo['type']) => {
    switch (type) {
      case 'error':
        return 'bg-red-900/20 border-red-800/30 text-red-800';
      case 'warning':
        return 'bg-yellow-900/20 border-yellow-800/30 text-yellow-800';
      case 'success':
  ...
  return (
    <div className="fixed top-4 right-4 z-50 space-y-2 max-w-sm">
      {errors.map((error) => (
        <div
          key={error.id}
          className={`border rounded-lg p-4 shadow-lg transition-all duration-300 transform translate-x-0 ${getStyles(error.type)}`}
        >
