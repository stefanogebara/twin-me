...
vi.mock('../../../api/middleware/auth.js', () => ({ authenticateUser: (req, _res, next) => { req.user = { id: '00000000-0000-4000-8000-000000000001' }; next(); } }));
vi.mock('../../../api/services/money/statements/accounts.js', async (original) => ({ ...(await original()), ownedStatementAccount: (...args) => f.owned(...args), checkStatementEvidence: (...args) => f.check(...args) }));
vi.mock('../../../api/services/money/store.js', async (original) => ({ ...(await original()), ingestSightings: (...args) => f.ingest(...args), refreshRecurring: vi.fn(async () => []), refreshReadings: vi.fn(async () => []) }));
import { StatementInputError } from '../../../api/services/money/statements/accounts.js';
