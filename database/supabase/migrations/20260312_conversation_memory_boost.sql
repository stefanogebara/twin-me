...
-- ============================================================================
-- 1. Update RPC fallback decay_rate for conversations: 3 → 14 days
-- 2. Update existing conversation memories: confidence 0.60 → 0.75, decay_rate 3 → 14
-- ============================================================================

-- Update existing conversation memories with new confidence and decay_rate
UPDATE user_memories
SET confidence = 0.75,
    decay_rate = 14
WHERE memory_type = 'conversation'
  AND (confidence < 0.75 OR decay_rate < 14 OR decay_rate IS NULL);

-- Recreate the search_memory_stream function with updated conversation decay default
-- Must drop first because we're redefining the function body
...
BEGIN
  ...
  normalized AS (
    SELECT
      c.*,
      -- Min-max normalize each factor across the candidate set
