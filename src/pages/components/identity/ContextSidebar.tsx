 ...
 * =============================================================
 * Soul Score ring at top, then tabbed content:
 *   - Soul: contributor cards (from SoulScore)
 *   - Insights: insight cards stacked vertically
 *   - Activity: recent memory correlations
 * "Chat with Twin" CTA at bottom.
 */
...
const ContextSidebar: React.FC<ContextSidebarProps> = ({ className = '' }) => {
  ...
  // Fetch recent activity for the Activity tab
    ...
    <div className={`flex flex-col gap-5 ${className}`}>
      ...
      <div className="min-h-[200px]">
        {activeTab === 'soul' && (
          ...
          >
            {/* Contributor cards are rendered inside SoulScore — already shown above */}
            <p
        ...
        {activeTab === 'activity' && (
          ...
          >
            ...
            {activityData?.memories && activityData.memories.length > 0 ? (
              activityData.memories.slice(0, 8).map((mem, i) => (
                <div
                  key={i}
                  className="flex items-start gap-2.5 px-3 py-2.5 rounded-[12px] transition-all duration-150 hover:bg-[rgba(255,255,255,0.04)]"
                >
      ...
      <button
        ...
        className="flex items-center justify-center gap-2 w-full px-4 py-3 rounded-[100px] text-sm font-medium transition-all duration-150 hover:opacity-85 active:scale-[0.98] mt-auto"
        style={{
