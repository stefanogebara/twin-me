 ...
 *   - Root cause: magic-link verify only set oauth_platform='magic_link' for
 *     NEW users. Existing users kept the stale 'google' value.
 *   - Fix: UPDATE users SET oauth_platform='magic_link', updated_at=NOW()
 ...
 * the singleFlight wrapper or the oauth_platform update.
 */
...
describe('Bug H11 — magic-link oauth_platform stale value fix', () => {
  it('magic-link verify updates oauth_platform for EXISTING users', () => {
    // Look for the else branch that handles existing users with an update
    // setting oauth_platform = 'magic_link'.
    const updatesExisting = /\.update\(\s*\{\s*oauth_platform:\s*['"]magic_link['"]/.test(authSimpleSrc);
    expect(updatesExisting).toBe(true);
  });

  it('magic-link verify still sets oauth_platform on user CREATE (regression guard)', () => {
    // The new-user insert path must still set 'magic_link' — don't lose
    // this on refactors.
    expect(authSimpleSrc).toMatch(/insert\(\s*\{[\s\S]{0,400}oauth_platform:\s*['"]magic_link['"][\s\S]{0,400}\}\s*\)/);
  });
  ...
  it('the existing-user update touches updated_at', () => {
    ...
    const pattern = /oauth_platform:\s*['"]magic_link['"][\s\S]{0,200}updated_at:/;
  ...
  it('the existing-user update targets users by id (not email)', () => {
    ...
    const updateBlock = authSimpleSrc.match(
      /\.update\(\s*\{\s*oauth_platform:\s*['"]magic_link['"][\s\S]{0,200}\.eq\(['"]id['"]/
    );
