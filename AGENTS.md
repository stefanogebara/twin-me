...
Global workflow rules live in `~/AGENTS.md` ("Workflow Orchestration" +
"Task Management") and `~/.Codex/rules/` — don't duplicate them here; the
last copy of this section drifted for months because it was a verbatim
mirror. Project-specific notes only:

- Durable plans for this repo: `.Codex/plans/<date>-<topic>/README.md`
...
Neural-inspired personality shaping for twin responses. Based on CL1_LLM_Encoder's biological neuron blending concept (`blended[tok] = (1-alpha) * model_probs + alpha * neural_probs`), adapted for API-level constraints (Codex via OpenRouter does NOT expose logprobs).
...
**Implementation Plan:** `.Codex/plans/2026-03-08-soul-signature-voting-layer.md` (10 tasks, 5 phases)
...
- **Schema** = AGENTS.md + expert personas (conventions for compilation)
...
- **AI**: OpenRouter (DeepSeek V3.2 for analysis, Mistral Small for extraction, Codex Sonnet for twin chat)
...
`.Codex/skills/intel/references/rubric.md` — nada entra nesses arquivos sem
passar por ela.
