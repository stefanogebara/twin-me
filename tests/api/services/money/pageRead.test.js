...
const f = vi.hoisted(() => ({ forecast: vi.fn(), months: vi.fn(), today: vi.fn(), ledger: vi.fn(), recurring: vi.fn(), readings: vi.fn(), categories: vi.fn(), usage: vi.fn(), accounts: vi.fn(), reconnect: vi.fn(), cards: vi.fn(), inbox: vi.fn() }));
...
describe('readPage', () => {
  it('reads every part once, and hands the forecast to the day instead of reading it twice', async () => {
    ...
    expect(Object.keys(data).sort()).toEqual(['accounts', 'capabilities', 'categories', 'forecast', 'inbox', 'ledger', 'months', 'readings', 'recurring', 'today', 'usage']);
