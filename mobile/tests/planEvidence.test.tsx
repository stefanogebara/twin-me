...
vi.mock('react-native',()=>({View:({children}:any)=><div>{children}</div>,ScrollView:({children}:any)=><div>{children}</div>,Pressable:({children,onPress}:any)=><button onClick={onPress}>{children}</button>,TextInput:()=>null,Alert:{alert:vi.fn()},Linking:{openURL:f.openURL},StyleSheet:{create:(s:unknown)=>s},AppState:{currentState:'active',addEventListener:()=>({remove(){}})}}));
...
vi.mock('../src/ui/primitives',()=>{const T=({children}:any)=><span>{children}</span>;return{Body:T,Heading:T,Label:T,List:T,Micro:T,Page:T,Small:T,Title:T,Hairline:()=>null,Pill:({label,onPress}:any)=><button onClick={onPress}>{label}</button>};});
