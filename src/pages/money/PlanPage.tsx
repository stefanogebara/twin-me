...
const monthLabel = (key: string) => new Date(`${key}-01T12:00:00Z`).toLocaleDateString('en-GB', { month: 'long', year: 'numeric' });
const dayName = (iso: string) => new Date(`${iso}T12:00:00Z`).toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long' });
      ...
      <div className="mv-shell">
        ...
        <div className="mv-col">
          <section className="mv-section mv-plan-top">
            <h1>{monthLabel(month).replace(/ \d{4}$/, '')}, day by day.</h1>
            <p className="mv-sub">{failed ? 'The plan could not be read right now.' : plan ? glyphs(plan.line) : ''}</p>
            <div className="mv-plan-months">
              <button type="button" className="mv-link" onClick={() => setMonth(shiftMonth(month, -1))}>{monthLabel(shiftMonth(month, -1)).replace(/ \d{4}$/, '')}</button>
              {month !== current ? <button type="button" className="mv-link" onClick={() => setMonth(shiftMonth(month, 1))}>{monthLabel(shiftMonth(month, 1)).replace(/ \d{4}$/, '')}</button> : null}
            </div>
          </section>

          {plan ? (
            <div className="mv-plan" role="grid" aria-label={`${monthLabel(month)}, day by day`}>
              {WEEKDAYS.map((w) => <span key={w} className="mv-plan-head" role="columnheader">{w}</span>)}
              ...
              {plan.cells.map((c) => {
                ...
                return (
                  <button
                    ...
                    aria-label={`${dayName(c.day)}: ${dayLine(c)}`}
          ...
          {cell ? (
            <section className="mv-section" aria-live="polite">
              <h2>{dayName(cell.day)}{cell.today ? ', today' : ''}.</h2>
              <p className="mv-sub">{dayLine(cell)}</p>
              {(cell.past || cell.today) && cell.rows.length ? (
                <ul className="mv-list">
                  {cell.rows.map((r) => (
                    <li key={r.id} className="mv-item mv-item--tight">
                      <span className="mv-item-text">
                        <span className="mv-item-title">{r.merchant || 'Unknown'}</span>
                        <span className="mv-item-sub">{timeOf(r.occurred_at)}</span>
                      </span>
                  ...
                  {cell.items.map((i, k) => (
                    <li key={k} className="mv-item mv-item--tight">
                      <span className="mv-item-text">
                        ...
                        <span className="mv-item-sub">{itemWords(i)}</span>
                      </span>
                  ...
                  <ul className="mv-list">
                    <li className="mv-item mv-item--tight">
                      <span className="mv-item-text">
                        ...
                        <span className="mv-item-sub">Your note. The ledger reads with it.</span>
                      </span>
                      <button type="button" className="mv-pill mv-pill--ghost" disabled={busy} onClick={() => void forgetNote()}><span>Forget</span></button>
                    </li>
                  </ul>
                ) : (
                  <form className="mv-plan-note-form" onSubmit={(e) => { e.preventDefault(); void saveNote(); }}>
                    <label className="mv-sr" htmlFor="plan-note">A note on this day</label>
                    <input id="plan-note" className="mv-field" type="text" autoComplete="off" maxLength={240} placeholder={cell.past ? 'What this day was' : cell.today ? 'What today is' : 'What this day will be: a trip, a visit, an exam'} value={draft} onChange={(e) => setDraft(e.target.value)} disabled={busy} />
                    <button type="submit" className="mv-pill mv-pill--ghost" disabled={busy || !draft.trim()}><span>Keep</span></button>
                  </form>
                )}
              </div>
            </section>
