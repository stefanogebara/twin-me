...
import { ArrowRight, Sparkles, User, Heart, Zap, LogIn } from 'lucide-react';
...
const Step1Welcome: React.FC<Step1WelcomeProps> = ({ onNext, goToStep }) => {
  const navigate = useNavigate();

  const handleBegin = () => {
    // Navigate to Create Account (Step 2)
    if (onNext) {
      onNext();
    } else {
  ...
  const handleLogin = () => {
    // Navigate directly to step 3 (About You) for existing users
    if (goToStep) {
      goToStep(3);
    } else {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-6 py-20 relative overflow-hidden" style={{ background: '#FAFAFA' }}>
      ...
      {/* Minimal Login/Sign Up buttons (Cofounder-inspired) */}
      <div className="fixed top-6 right-6 z-50 flex items-center gap-4">
        <button
          onClick={handleLogin}
          className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-stone-600 hover:text-stone-900 transition-colors duration-200"
        >
        ...
        <button
          onClick={handleBegin}
          className="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-medium text-white bg-stone-900 rounded-lg hover:bg-stone-800 transition-all duration-200 shadow-sm hover:shadow-md"
        >
          Sign Up
...
console.log('✅ Step1Welcome.tsx updated with Login/Sign Up buttons!');
console.log('   - Added minimal Login/Sign Up buttons in top-right corner (Cofounder pattern)');
console.log('   - Log In: Text button with icon → navigates to Step 3 (About You)');
console.log('   - Sign Up: Solid button → navigates to Step 2 (Create Account)');
console.log('   - Updated handleBegin to navigate to /onboarding/create-account');
console.log('   - Added LogIn icon import from lucide-react');
console.log('   - Buttons use stone-900 background with hover:stone-800');
