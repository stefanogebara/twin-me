...
-- Migration 002: Data-Driven Digital Twins
...
CREATE TABLE IF NOT EXISTS data_connectors (
  ...
  INDEX idx_connectors_user_active (user_id, is_active),
  INDEX idx_connectors_sync_due (last_sync, sync_frequency) WHERE is_active = true
);
...
CREATE TABLE IF NOT EXISTS user_data_raw (
  ...
  -- Indexes
  INDEX idx_raw_data_user_type (user_id, data_type),
  INDEX idx_raw_data_processing (processed, processing_error) WHERE NOT processed,
  INDEX idx_raw_data_retention (retention_until) WHERE retention_until IS NOT NULL,
  INDEX idx_raw_data_temporal (source_timestamp DESC)
);
...
CREATE TABLE IF NOT EXISTS personality_insights (
  ...
  INDEX idx_insights_confidence (confidence_score DESC),
  INDEX idx_insights_validity (valid_from, valid_until) WHERE valid_until IS NULL,
  INDEX idx_insights_evolution (supersedes_insight_id) WHERE supersedes_insight_id IS NOT NULL
);
...
CREATE TABLE IF NOT EXISTS twin_evolution_log (
  ...
  -- Indexes
  INDEX idx_evolution_twin (twin_id, created_at DESC),
  INDEX idx_evolution_changes (change_type, created_at DESC)
);
...
CREATE TABLE IF NOT EXISTS automation_rules (
  ...
  INDEX idx_automation_type (rule_type, is_active)
);
...
CREATE TABLE IF NOT EXISTS data_quality_metrics (
  ...
  INDEX idx_quality_metrics_date (metric_date DESC)
);
...
CREATE TABLE IF NOT EXISTS sync_queue (
  ...
  -- Indexes
  INDEX idx_sync_queue_processing (status, priority DESC, scheduled_for ASC) WHERE status = 'pending',
  INDEX idx_sync_queue_user (user_id, created_at DESC),
  INDEX idx_sync_queue_cleanup (completed_at) WHERE status IN ('completed', 'failed')
);
...
CREATE VIEW user_data_sources AS
SELECT
  user_id,
...
CREATE VIEW recent_twin_changes AS
SELECT
  tel.*,
...
CREATE VIEW data_ingestion_health AS
SELECT
  user_id,
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_raw_data_user_recent
ON user_data_raw (user_id, source_timestamp DESC)
WHERE source_timestamp >= NOW() - INTERVAL '30 days';

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_insights_user_recent
ON personality_insights (user_id, last_updated DESC)
WHERE valid_until IS NULL;

-- Insert default automation rules for new users
...
-- Migration complete
-- Next steps: Implement data connector interfaces and OAuth integration