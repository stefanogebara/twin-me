...
function score(s, reply, ms) {
  ...
  /* a computed reply's sums (a share, a total of what comes back) are the ledger's own arithmetic, not a line to quote */
  checks.grounded = Boolean(reply.computed) || amounts.every((a) => amountsInText(basis).some((b) => Math.abs(a - b) < 0.005) || typed.some((b) => Math.abs(a - b) < 0.005)) || (amounts.length > 0 && !reply.basis && kinds.length > 0);
...
async function ask(token, message, history, stream) {
  ...
  for (const block of (await r.text()).split('\n\n')) {
    ...
    else if (ev.phase === 'actions') { got.actions = ev.actions || []; got.receipts = ev.receipts || []; got.basis = ev.basis || []; got.computed = Boolean(ev.computed); }
...
(async () => {
  ...
  const runOne = async (s, step, history, label) => {
    ...
    const jd = reply && JUDGE && scenario.route !== 'short' && scenario.judge !== false && !computed && !step.act ? await grade(scenario, reply, reply.thinking) : null;
