...
describe('parseNarrative', () => {
  it('reads a mobile-wallet payment as a shop on a card', () => {
    expect(parseNarrative('PAGO MOVIL EN EL CORTE INGLES, MADRID ES, TARJ. :*554444')).toEqual({
      merchant: 'El Corte Ingles', channel: 'card', cardLast4: '4444', isRefund: false,
    });
    expect(parseNarrative('PAGO MOVIL EN RENFE CERCANIAS, MADRID ES, TARJ. :*554444').merchant).toBe('Renfe Cercanias');
  });

  it('reads a card purchase and drops the reference the bank glues on', () => {
    expect(parseNarrative('COMPRA Cabify ES 2636eRmHs0tq, Madrid, TARJETA 5555555555554444 , COMISION 0,00')).toEqual({
      merchant: 'Cabify', channel: 'card', cardLast4: '4444', isRefund: false,
    });
    expect(parseNarrative('COMPRA PLAYTOMIC.IO 8C575A28, MADRID, TARJETA 5555555555554444 , COMISION 0,00').merchant).toBe('Playtomic.io');
    expect(parseNarrative('COMPRA Spotify P4675EFF52, Stockholm, TARJETA 5555555555554444 , COMISION 0,00').merchant).toBe('Spotify');
    expect(parseNarrative('COMPRA 31AUG MRRYNFLV, Barcelona, TARJETA 5555555555554444 , COMISION 0,00').merchant).toBe('Mrrynflv');
  });

  it('cuts a reference off a domain and a letter the bank truncated', () => {
    expect(parseNarrative('COMPRA BOLT.EU/O/2609041118, Tallinn, TARJETA 5555555555554444 , COMISION 0,00').merchant).toBe('Bolt.eu');
    expect(parseNarrative('COMPRA AB Servicios Selecta E, Madrid, TARJETA 5555555555554444 , COMISION 0,00').merchant).toBe('AB Servicios Selecta');
    /* IE is Ireland's country code and also the name of a Madrid business school. */
    expect(parseNarrative('PAGO MOVIL EN TORRE IE, VIEJAS ES, TARJ. :*554444').merchant).toBe('Torre IE');
  });

  it('keeps the merchant, not the aggregator, when one prints its own name', () => {
    expect(parseNarrative('COMPRA SQ *SHAKE SHACK, Boston, TARJETA 5555555555554444 , COMISION 0,00').merchant).toBe('Shake Shack');
    /* FACEBK is Facebook's card descriptor, and nobody says "Facebk". */
    expect(parseNarrative('COMPRA FACEBK *Q42J82JD24, DUBLIN 2, TARJETA 5555555555554444 , COMISION 0,00').merchant).toBe('Facebook');
  });

  it('marks a refund and still names the shop', () => {
    const r = parseNarrative('DEVOLUCION COMPRA EN ARBITRADE MADRID 3, GETAFE, TARJETA 5555555555554444 , COMISION 0,00');
  ...
  it('drops a legal form from the end of a company name', () => {
    expect(parseNarrative('COMPRA HIGGSFIELD INC., SAN FRANCISCO, TARJETA 5555555555554444 , COMISION 0,00').merchant).toBe('Higgsfield');
    expect(parseNarrative('COMPRA VERCEL INC., COVINA, TARJETA 5555555555554444 , COMISION 0,00').merchant).toBe('Vercel');
  });
...
describe('cardFrom', () => {
  it('keeps four digits from either way the card is printed', () => {
    expect(cardFrom('TARJ. :*554444')).toBe('4444');
    expect(cardFrom('TARJETA 5555555555554444 , COMISION 0,00')).toBe('4444');
...
describe('cityFrom', () => {
  it('reads the town out of a card purchase, country code and all', () => {
    expect(cityFrom('PAGO MOVIL EN EL CORTE INGLES, MADRID ES, TARJ. :*554444')).toBe('Madrid');
    expect(cityFrom('COMPRA Cabify ES 2636eRmHs0tq, Madrid, TARJETA 5555555555554444 , COMISION 0,00')).toBe('Madrid');
    expect(cityFrom('PAGO MOVIL EN EXPVILLANUEVA, ALCOBENDAS ES, TARJ. :*554444')).toBe('Alcobendas');
    /* The bank truncates the town and glues the country to it. */
    expect(cityFrom('PAGO MOVIL EN SIMPLY ALCALA, PARQUE RETIROES, TARJ. :*554444')).toBe('Parque Retiro');
    expect(cityFrom('COMPRA FACEBK *Q42J82JD24, DUBLIN 2, TARJETA 5555555555554444 , COMISION 0,00')).toBe('Dublin');
    /* Some merchants print a legal form where the town would be. */
    expect(cityFrom('COMPRA VERCEL, INC., COVINA, TARJETA 5555555555554444 , COMISION 0,00')).toBe('Covina');
  });
