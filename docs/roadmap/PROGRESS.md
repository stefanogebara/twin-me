...
| M2-2b | The three functions still over 250 lines: `MoneyConversation` (325), `MoneySetupPage` (409), `readingWords` (375) | each < 250; the goal test ratchets them | todo |
| M2-3 | One read per view, cached per ingestion revision | Today cold < 1.5 s | todo — measured 2026-09-19 on the local build: 30 `/api/money` requests to paint Today, 31 Month, 38 You; a Today → Month → Today switch sends 0 (the snapshot holds) |
