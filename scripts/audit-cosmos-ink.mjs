 ...
 *   ROOT=.ld                 the page's root element (default #root, main's
 *                            app root; the Cosmos branch's surfaces are
 *                            .presence-cosmos, its front door .ld)
...
const OUT = process.argv[4] || '/tmp/register-ink-audit';
/* On main every colour resolves to src/styles/register.css, declared on :root,
   so the defaults audit the register's quiet ink there. On the Cosmos branch
   pass TOKEN=--c-ink-3 SCOPE=.presence-cosmos ROOT=.presence-cosmos. */
const TOKEN = process.env.TOKEN || '--rg-ink-3';
/* SCOPE: where the sentinel goes. The default measures the shared token; a
   scoped override (.mv, .pc-demo-stage) is audited by naming its element. */
...
/* ROOT: the element every judged run lives under, and the one a route must
   render before it is measured. Main renders everything under #root; on the
   Cosmos branch the surfaces are .presence-cosmos and the front door .ld. */
...
/* Main's public routes by default, so a signed-out pass covers something; /auth
   and /waitlist only render when signed OUT. For a signed-in pass name the
   routes: ROUTES=/today,/settings,/money. (The Cosmos branch's list was
   /presence, /presence/login, /auth, /waitlist, /cosmos/system, /cosmos/demos,
   /cosmos/landing, /portrait, /demo, /presence/home, /presence/onboarding.) */
const ROUTES = (process.env.ROUTES || [
  '/', '/auth', '/waitlist', '/beta', '/privacy-policy', '/terms',
  '/presence', '/presence/login', '/system',
].join(',')).split(',');
...
if (VERIFY !== 'none') {
  ...
  /* On http 127.0.0.1 the refresh cookie does not survive, and the access token
     lives in sessionStorage, which a new page does not inherit. Every route
     below gets a fresh page, so hand each one the signed-in page's session
     before its scripts run, or every gated route bounces to /auth. */
  ...
  await ctx.addInitScript(dump => {
    for (const [k, v] of Object.entries(JSON.parse(dump))) if (sessionStorage.getItem(k) === null) sessionStorage.setItem(k, v);
  }, session);
} else {
