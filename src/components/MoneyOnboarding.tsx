...
function BanksStep({ accounts, onNext }: { accounts: MoneyAccount[]; onNext: () => void }) {
  ...
  const [note, setNote] = useState<string | null>(() => {
    ...
    return o === 'connected' ? `${bankLabel(p.get('name'))} is connected. The first read is on its way.` : `The bank connection did not go through${why ? ` (the bank said: ${why})` : ''}. Try it again.`;
  });
  async function connect(bank: string) {
    ...
    catch { setNote('The bank did not answer. Try again in a moment.'); setBusy(null); }
  }
  ...
  return (
    <div className="mv la" role="dialog" aria-modal="true" aria-labelledby="la-title">
      <div className="la-col">
        <h1 id="la-title">Connect your bank.</h1>
        <p className="mv-sub">It reads what comes in and goes out, four times a day. It can never move money. Connect as many as you use, then Next.</p>
            ...
            return (
              <li key={b.name} className="mv-item">
                <span className="mv-item-text">
                  ...
                  <span className="mv-item-sub">{rows.length ? rows.map((a) => `${a.name || 'Account'} ${a.iban_mask || ''}`.trim()).join(', ') : 'Read four times a day. You confirm it every six months.'}</span>
                </span>
                <span className="mv-item-end">
                  {rows.length ? <span className="mv-quiet">Connected</span> : <button type="button" className="mv-pill mv-pill--ghost" disabled={busy !== null} onClick={() => void connect(b.name)}>{busy === b.name ? 'Opening' : 'Connect'}</button>}
                </span>
              </li>
            );
          })}
        ...
        <div className="mv-ctas">
          {accounts.length ? <button type="button" className="mv-pill" onClick={onNext}>Next</button> : <button type="button" className="mv-pill mv-pill--ghost" onClick={onNext}>Not now</button>}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="mv la" role="dialog" aria-modal="true" aria-labelledby="la-title">
      <div className="la-col la-col--wide">
        <h1 id="la-title">Where your days happen.</h1>
        <p className="mv-sub">Three places the ledger reads your month against: the walk-to shops, the exam weeks, the salary. Each is a fact you can forget later under Settings.</p>
        <ul className="mv-list">
          {SLOTS.map((s) => <PlaceSlot key={s.kind} slot={s} fact={facts.find((f) => f.kind === s.kind) || null} onKept={onKept} />)}
        </ul>
        <div className="mv-ctas">
          <button type="button" className={`mv-pill${facts.some((f) => SLOTS.some((s) => s.kind === f.kind)) ? '' : ' mv-pill--ghost'}`} onClick={onNext}>{facts.some((f) => SLOTS.some((s) => s.kind === f.kind)) ? 'Next' : 'Not now'}</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <li className="la-slot">
      <div className="mv-item">
        <span className="mv-item-text">
          <span className="mv-item-title">{slot.title}</span>
          <span className="mv-item-sub">{kept ? String(fact.value || fact.subject) : slot.hint}</span>
        </span>
        <span className="mv-item-end">{kept ? <button type="button" className="mv-pill mv-pill--ghost" onClick={() => setEditing(true)}>Change</button> : null}</span>
      </div>
      {!kept ? (
        <div className="la-search">
          <label className="mv-sr" htmlFor={`la-${slot.kind}`}>{slot.title}</label>
              ...
              {hits.map((h) => (
                <li key={h.id}>
                  <button type="button" className="mv-item la-hit" disabled={busy} onClick={() => void keep(h)}>
                    ...
                    <span className="mv-item-end mv-quiet">Keep</span>
                  </button>
                </li>
              ))}
            </ul>
