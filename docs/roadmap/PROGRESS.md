...
| M3-2 | Lazy-load every non-money route; drop `vendor-elevenlabs` from the money bundle | doing (Claude, 2026-09-19) — slices 1–3 done (#434): PostHog on idle behind an ordered queue (209 KB out), each dictionary only for its language with a first-frame guarantee (119 KB out; the entry chunk is 230 KB, was 556), and slice 1: the start no longer prefetches the legacy twin's TodayPage, TalkToTwin and MoneyPage (with recharts) at 0 ms; only the money siblings are warmed on idle; the money screen loads 13 chunks, 1 070 KB, none of them legacy; `vendor-elevenlabs` is reached only from the onboarding's DeepInterview/SoulOrb (lazy). Next, measured from the sourcemap: the 556 KB entry chunk is posthog-js 209 KB, the two money dictionaries 119 KB (both languages for everyone), App.tsx 35, OAuthCallback 29 (eager route), AuthContext 25, enrichmentService 19, NocturneLanding 13 — slice 2 loads PostHog on idle, the reader's dictionary only, and the eager routes lazily |
