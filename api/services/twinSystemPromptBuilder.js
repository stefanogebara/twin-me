...
export function buildTwinSystemPrompt(soulSignature, platformData, twinSummary = null, proactiveInsights = null, userLocation = null, coreMemoryBlockText = null, departmentProposals = null, wikiPages = null, directives = null) {
  ...
  const now = new Date();
  // Use user's timezone if available, otherwise UTC as final fallback
  const tzOffset = userLocation?.timezone
    ? (() => { try { const parts = new Intl.DateTimeFormat('en-US', { timeZone: userLocation.timezone, hour: 'numeric', hour12: false }).formatToParts(now); return parseInt(parts.find(p => p.type === 'hour')?.value || '0', 10); } catch { return now.getUTCHours(); } })()
    : now.getUTCHours();
  const hour = tzOffset;
  const dayOfWeek = now.toLocaleDateString('en-US', { weekday: 'long', timeZone: userLocation?.timezone || 'UTC' });
  ...
  if (platformData) {
    ...
    if (platformData.discord?.analytics?.summary) {
      dynamicContext += `\n\nMy Discord activity:`;
      dynamicContext += `\nDiscord data for the period asked about (use these exact numbers when answering): ${platformData.discord.analytics.summary}`;
    }

    if (platformData.linkedin?.analytics?.summary) {
      dynamicContext += `\n\nMy LinkedIn activity:`;
      dynamicContext += `\nLinkedIn data for the period asked about (use these exact numbers when answering): ${platformData.linkedin.analytics.summary}`;
    }
