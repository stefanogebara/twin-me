...
/**
 * Helper function to verify Gmail access
 */
async function verifyGmailAccess(userId) {
  try {
    ...
    if (!messagesResponse.ok) {
      if (messagesResponse.status === 401) {
        return { error: 'Gmail access token expired. Please reconnect.' };
      }
      throw new Error(`Gmail API error: ${messagesResponse.status}`);
    }
    ...
    // Fetch details for first 3 messages
    if (messagesData.messages && messagesData.messages.length > 0) {
...
async function verifyCalendarAccess(userId) {
  try {
    ...
    if (!calendarsResponse.ok) {
      if (calendarsResponse.status === 401) {
        return { error: 'Calendar access token expired. Please reconnect.' };
      }
      throw new Error(`Calendar API error: ${calendarsResponse.status}`);
    }
