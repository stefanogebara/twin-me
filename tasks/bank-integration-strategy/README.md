...
- Add inbound document/media handling to `api/_app/routes/whatsapp-kapso-webhook.js` (currently `msg.type !== 'text'` returns null — attachments silently drop). Route OFX/CSV/XLSX attachments to `parserDispatcher.js` → `ingestRawTransactions`.
...
- Required: an explicit per-user consent toggle, because reading bodies/attachments departs from the current metadata-only posture in `api/_app/services/observationFetchers/gmail.js`; update the OAuth consent-screen description if it currently says metadata-only. Note the per-transaction-email idea is dead — BR banks alert via push/SMS/WhatsApp, not email (Itau's per-purchase alert is a paid R$7.99/mo SMS) — this phase is statement pickup, not a live feed.
