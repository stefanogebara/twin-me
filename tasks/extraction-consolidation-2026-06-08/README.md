...
- **DONE:** `ingestion_source` telemetry — new `api/_app/services/extractionTelemetry.js` emits a single greppable `extraction_run` event with a frozen `INGESTION_SOURCE` enum (`background`/`post_onboarding`/`on_demand`/`oauth_callback`). Wired into all four dispatch points (P1 cron + post-onboarding, P2 orchestrator, P3 dataExtractionService). Pure logging, no behavior change. Aggregate prod logs by `ingestion_source`+`platform` to see which paths actually fire per platform before any deletion in P3-P5.
